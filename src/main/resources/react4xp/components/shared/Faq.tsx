...
export const Faq: React.FC<FaqProps> = ({
  ...
  return (
    <div
      ...
      id={id}
      className="bg-menu-bg border border-[rgba(74,16,74,0.1)] rounded-[3px] shadow-[0px_2px_10px_rgba(74,16,74,0.14)] overflow-hidden [&>input[type=checkbox]:checked~label_.arrowPurple]:rotate-[360deg] [&>input[type=checkbox]:checked~label_.arrowPurple]:transition-transform [&>input[type=checkbox]:checked~label_.arrowPurple]:duration-500 [&>input[type=checkbox]:checked~label_.arrowPurple]:linear [&>input[type=checkbox]:checked~div[itemprop=acceptedAnswer]]:max-h-[500px] [&>input[type=checkbox]:checked~div[itemprop=acceptedAnswer]]:transition-[max-height] [&>input[type=checkbox]:checked~div[itemprop=acceptedAnswer]]:duration-[0.25s] [&>input[type=checkbox]:checked~div[itemprop=acceptedAnswer]]:ease-in"
    >
      ...
      <label htmlFor={`checkbox-${itemID}`}>
        ...
        >
          {headerItem}
          <div className="arrowPurple rotate-180 w-0 h-0 border-l-[15px] border-l-transparent border-r-[15px] border-r-transparent border-b-[22px] border-b-primary-700 rounded-[5px] transition-transform duration-500 linear" />
        </div>
      </label>
      <div
        itemScope
